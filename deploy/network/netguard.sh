...
CAPGATE_HOST="${CAPGATE_HOST:-harness}"
CAPGATE_PORT="${CAPGATE_PORT:-8080}"

i=0
until CAPGATE_IP="$(getent hosts "$CAPGATE_HOST" | awk '{print $1; exit}')" && [ -n "$CAPGATE_IP" ]; do
  i=$((i + 1)); [ "$i" -gt 60 ] && { echo "netguard: cannot resolve $CAPGATE_HOST" >&2; exit 1; }
  sleep 0.5
done
...
iptables -A OUTPUT -p tcp -d "$CAPGATE_IP" --dport "$CAPGATE_PORT" -j ACCEPT
...
echo "netguard: agent egress limited to tcp://$CAPGATE_IP:$CAPGATE_PORT"
iptables -S OUTPUT
