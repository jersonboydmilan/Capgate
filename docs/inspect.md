...
```bash
harness inspect --task examples/simulation/task.yaml --audit audit.jsonl \
...
configured approver — treat `harness inspect` like any local admin console.

## With the Docker deployment

```bash
cd deploy && docker compose up -d harness
docker compose exec harness python3 -m harness.cli token issue --keyring /secrets/token_keyring \
    --sub alice --role approver --ttl 1h > alice.token
docker compose cp harness:/data/audit.jsonl ./audit.jsonl
harness inspect --audit audit.jsonl --harness-url http://127.0.0.1:8080 --approver-token-file alice.token
```
